...
class AppiumServer:
    ...
    def start_server(self, appium_port, bootstrap_port, appium_address, appium_log_file):
        # 启动appium
        # todo：根据devices的数量进行appium server启动
        logger.info("----启动appium server----\n")
        self._port = appium_port
        cmd = "appium -a %s -p %s -bp %s" % (appium_address, appium_port, bootstrap_port)
        with open(appium_log_file, "a") as appium_log:
            self.popen_appium = subprocess.Popen(cmd, shell=True,
                        stdout=appium_log,
                                                 stderr=subprocess.STDOUT)

        resp = self.detection_communication(appium_port, bootstrap_port, appium_address, appium_log_file)

        if resp['status'] != 0:
            self.start_server(appium_port, bootstrap_port, appium_address, appium_log_file)
    ...
    def detection_communication(self, appium_port, bootstrap_port, appium_address, appium_log_file):
        # 检测appium server是否已启动完成
        try:

            url = f"http://{appium_address}:{appium_port}/wd/hub/status"
